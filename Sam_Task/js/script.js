...
const buyBtn = document.querySelectorAll(".products__button"); 
 
buyBtn.forEach(function (elem) { 
  elem.addEventListener("click", function () { 
    ids = JSON.parse(localStorage.getItem("ids")); 
    if (ids == null || ids.length == 0) { 
      document.getElementById("checkoutButton").style.pointerEvents = "auto"; 
    } 
    if (ids == null || ids.indexOf(this.parentNode.id) < 0) 
      addNewCartItem(this.parentNode.id); 
    else incCountCartItem(this.parentNode.id); 
  }); 
});